    ...
    public class InsertionEventSystem : MonoBehaviour {
        ...
        public void registerListener(Type type, string eventName, string listenerName, MulticastDelegate listener) {
            ...
            if (types.Length != eventList[type][eventName].Length) {
                Debug.LogError(
                    $"Event {eventName} of type {type.ToString()} has {eventList[type][eventName].Length} parameters, but the listener has {types.Length} parameters!");
                return;
            }
            ...
            for (int i = 0; i < types.Length; i++) {
                ...
                if (eventList[type][eventName][i] == null ||
                    types[i].IsAssignableFrom(eventList[type][eventName][i])) continue;
                Debug.LogError(
                    $"Event {eventName} of type {type.ToString()} has parameter {i} of type {eventList[type][eventName][i].ToString()}, but the listener has parameter {i} of type {types[i].ToString()}!");
                return;
            }
        ...
        public void triggerListener(Type type, string eventName, params object[] objects) {
            ...
            foreach (MulticastDelegate listener in typeEvent[type][eventName].Values) {
                try {
                    listener.DynamicInvoke(objects);
                }
                catch (Exception e) {
