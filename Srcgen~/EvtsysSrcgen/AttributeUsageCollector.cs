...
namespace Fries.EvtsysSrcgen {
    ...
    public class AttributeUsageCollector : ISourceGenerator {
        ...
        public void Initialize(GeneratorInitializationContext context) {
            context.RegisterForSyntaxNotifications(() => new EvtListenerReceiver());
        }
        ...
        public void Execute(GeneratorExecutionContext context) { 
            ...
            try {
                if (!(context.SyntaxReceiver is EvtListenerReceiver receiver)) {
                    log(assemblyName, "Source Generator quit working because this is not the correct SyntaxReceiver!");
                    return;
                }

                Compilation compilation = context.Compilation;
                object symbol = compilation.GetTypeByMetadataName("Fries.EvtSystem.EvtListener");
                if (symbol == null) {
                    log(assemblyName, "Cannot find EvtListener type, did you delete it by accident? Terminating...");
                    return;
                }

                INamedTypeSymbol targetAttrSymbol = (INamedTypeSymbol)symbol;
                ...
                foreach (var method in receiver.candidateMethods) {
                    ...
                    if (methodInfo.DeclaredAccessibility == Accessibility.Public) {
                        ...
                        if (methodInfo.Parameters.Length > 0) {
                            ...
                            sb.AppendLine(
                                $"            registerEvtListenerByInfo(\"{methodName}\", typeof({classFullName}), listener, new Action<{paramTypeFullName}>({classFullName}.{methodName}));");
                        }
                        else
                            sb.AppendLine(
                                $"            registerEvtListenerByInfo(\"{methodName}\", typeof({classFullName}), listener, new Action({classFullName}.{methodName}));");
                    }
                    else {
                        string bindingFlags = "BindingFlags.NonPublic | BindingFlags.Static";
                        sb.AppendLine(
                            $"            registerEvtListenerByReflection(typeof({classFullName}).GetMethod(\"{methodName}\", {bindingFlags}));");
                    }
